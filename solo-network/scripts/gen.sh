...
if [ "2.0.0" != "$(cryptogen version | grep Version | awk -F ': ' '{print $2}')" ] ; then
    echo -e "\033[31m cryptogen version need 2.0.0\033[0m"
    exit 1
fi
# 检测configtxgen和版本
...
if [ "2.0.0" != "$(configtxgen --version | grep Version | awk -F ': ' '{print $2}')" ] ; then
    echo -e "\033[31m configtxgen version need 2.0.0\033[0m"
    exit 1
fi
# 生成证书文件
